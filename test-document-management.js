...
This document verifies that the file upload system is working correctly.`;
  ...
  try {
    ...
    form.append('documentId', `test-doc-${Date.now()}`);
    ...
    if (!response.ok) {
      return { success: false, error: `Upload failed with status ${response.status}: ${JSON.stringify(jsonData)}` };
    }
...
async function testDocumentWorkflow() {
  ...
  const response = await makeRequest(`${BASE_URL}/api/documents`);
  if (!response.ok) {
    return { success: false, error: 'Failed to fetch documents after signing' };
  }
  ...
  if (document.status !== 'signed') {
    return { success: false, error: `Document status is ${document.status}, expected 'signed'` };
  }
  
  return { 
    success: true, 
    details: `Complete workflow successful: Created → Signed → Verified (Status: ${document.status})` 
  };
}
...
async function runAllTests() {
  ...
  // Test 4: File Upload
  await runTest('File Upload - POST /api/documents/upload', testFileUpload);
